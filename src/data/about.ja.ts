...
export const data: Data = {
  ...
  experience: [
    {
      ...
      endDate: "",
      projects: {
        tags: ["Golang", "NextJS", "ScyllaDB", "k9s", "ArgoCD", "Kafka", "Spark", "Trino", "Iceberg", "Parquet", "Scrum", "Claude Code", "AI活用"],
        job: "ビックデータグループ / バックエンドチーム → xfunctionチーム",
        jobDescription:
          "ECプラットフォームのビックデータグループでバックエンドエンジニアとして従事。最初はバックエンドチームに所属し、その後xfunctionチームに異動。Golang、NextJS、ScyllaDB、k9s、ArgoCD、Kafka、Sparkなどを利用してシステムの開発・保守を担当。スクラム開発で開発を進めている。xfunctionチームの立ち上げに初期メンバーとして中心的な役割を果たす。従来はFE・BEのコミュニケーションや他部署との連携がチグハグで、直近のPJでは6月リリース予定のものが10月にリリースされるなどコミュニケーションロスによる遅延が発生していた。新チームの創設にあたり、心理的安全性を重視し、ドラッカー風エクササイズや積極的なチーム・メンバー間でのコミュニケーション、PdM・ステークホルダーとの期待値のすり合わせを実施。これによりチーム設立5ヶ月で新規チームにおける意図しないPJの遅延を0にした。デザイナー、他部署、他チーム間で積極的にコミュニケーションを取り、xfunction内のリードとして活動。データ基盤にIceberg（保存形式Parquet）を採用し、分析クエリはTrinoで実行。AIを駆使してコードを精査し、Spark SQLのデータ取り込み処理に`CREATE OR REPLACE TABLE`構文を導入することで、処理速度・コストを30%改善した。AI活用を推進し、Claude CodeのAgents（サブエージェント）とハーネスの最適化に取り組む。タスクの性質に応じてOpus・Sonnet・Haikuを使い分けるサブエージェント構成を設計・導入し、トークン使用料を50%削減した。",
        experience: ["ビッグデータインフラ", "Kafkaによるイベント駆動アーキテクチャ", "Kubernetesオーケストレーション", "ArgoCDによるCI/CD", "スクラム開発", "Spark最適化（CREATE OR REPLACE TABLE導入で処理速度・コスト30%改善）", "AI活用推進（Claude Code Agents・ハーネス最適化）", "Opus・Sonnet・Haikuの使い分けによるサブエージェント設計", "トークン使用料50%削減", "チーム立ち上げ（初期メンバー・中心的役割）", "コミュニケーションロスによる遅延の解消", "スクラムイベント運営", "心理的安全性向上施策", "ドラッカー風エクササイズ", "PdM・ステークホルダーとの期待値調整", "リモートナレッジシェア", "クロスファンクショナルコミュニケーション", "リーダーシップ"],
        team: 0,
      },
    },
    {
      ...
      projects: {
        ...
        experience: ["高トラフィックシステムアーキテクチャ", "マイクロサービスアーキテクチャ", "データ分析基盤構築（Parquet/Athena/Glue/Quick Suite）", "サーバーレスによるコスト最適化（30%削減）", "AI導入リーダーシップ", "IaCによるAIフレンドリーなインフラ構築", "AWS SavingsPlan最適化", "Redisによるパフォーマンス最適化", "顧客対応型テックリーダーシップ", "画像処理と座標マッピング", "2週間でのインフラ構築・リリース", "ベトナム現地コラボレーション", "英語でのチームマネジメント", "短期間でのフルスタック開発"],
        team: 16,
      },
    },
    {
  ...
  selfProject: [
    {
      title: "BikeHub",
      desc: `バイクの燃費記録とニュース閲覧ができるモバイルアプリ。Java・JSP/Servlet製の前身サービス「バイク燃費.com」を個人で開発・運用したのち、Django (DRF) のバックエンドAPIとReact Native (Expo) で後継としてフルリニューアルし、iOS/Android両ストアで公開・運用した。現在はサービス終了。`,
      link: "",
      tags: [
        "Python(Django)",
        "DRF",
    ...
    {
      title: "SAVE EAT",
      desc: `コロナ禍の飲食店を支援する応援サイト。企画から1週間で開発・公開し、Docker Composeで運用した。現在はサービス終了。`,
      link: "",
