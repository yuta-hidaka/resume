...
- **企業サイトCMS移行 (2024-04 – 2024-11)**: カスタムCMSをヘッドレス化し、Lambda/CloudFront+IaCで再構築、AstroでSEO/表示速度改善。  
- **在庫管理システム刷新 (2022-09 – 2023-10)**: 海外チームと非同期開発でバックエンド/フロント/インフラを一貫担当。  
- PBI作成からリリース運用までスクラムを実践し、モブプロ/ペアプロで育成と属人化排除を推進。  
- 画像最適化/CDN/キャッシュ設計やCloud Runチューニングで高負荷時の安定性を確保。  
...
- e-Gov要件を調査し、法務ドメインの要件をGo/Svelte実装に落とし込みリリース。  
...
- 150GB超のDynamoDBに対してクエリ/インデックスを改善し、主要画面のレスポンスを安定化。  
- ログ/デバッグ手順を整備し、オンボーディング工数を削減。  
...
- Alteryxでデータ秘匿化・取り込み・クレンジング処理を設計し、移行ログとバッチ連携を実装。  
- Outlook API連携の予実管理アプリでフロント/バックを担当し、業務可視化を推進。  

### Sumitomo Chemical — R&D (2012-04 – 2018-03)
- PPフィルム、バッテリーセパレータ、医療グレード材料の研究開発。  
- 射出成形や材料解析の実験設計・評価を通じて、品質改善とプロセス最適化を経験。  

### Independent Business (2018-04 – 2018-09)
- スペイン産生ハムの輸入販売を企画・交渉・テスト輸入まで実施。  
- サプライチェーン構築と価格交渉、通関・検査プロセスの実務を経験。  
